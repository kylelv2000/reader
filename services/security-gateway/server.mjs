...
// Zero config: reader-core generates the session secret in the shared storage
// volume (mounted read-only here). An explicit env var still wins; if the
// file never appears we fall back to a per-process secret with a warning.
const sessionSecretFile = process.env.YOMU_SESSION_SECRET_FILE || "/storage/data/session-secret.txt";
let sessionSecret = process.env.YOMU_SESSION_SECRET || "";
let fallbackSecret = "";
function currentSessionSecret() {
  if (sessionSecret) return sessionSecret;
  try {
    sessionSecret = readFileSync(sessionSecretFile, "utf8").trim();
  } catch { /* core not started yet */ }
  if (sessionSecret) return sessionSecret;
  if (!fallbackSecret) {
    fallbackSecret = randomBytes(48).toString("base64url");
    process.stderr.write(`session secret file ${sessionSecretFile} is not readable; sessions will reset on restart\n`);
  }
  return fallbackSecret;
}
// A request that arrived over HTTPS (directly or via a reverse proxy that
// sets x-forwarded-proto) gets Secure cookies and HSTS automatically.
...
export function applySecurityHeaders(response, appShell = false, secure = false) {
...
function cookie(name, value, { httpOnly = false, maxAge = sessionTtlSeconds, secure = false } = {}) {
  return [
...
function clearCookies(rawCookie = "", secure = false) {
  const sourceCookies = rawCookie.split(";").flatMap((part) => {
    ...
    return [`${name}=; Path=/reader3/bookSourceProxy; Max-Age=0; HttpOnly; SameSite=Lax${secure ? "; Secure" : ""}`];
  });
  return [
    cookie(sessionCookieName, "", { httpOnly: true, maxAge: 0, secure }),
    cookie(csrfCookieName, "", { maxAge: 0, secure }),
    ...sourceCookies,
  ];
}
...
// Same-origin check without configuration: the browser sets both Origin and
// Host and neither can be forged cross-site, so an exact host match means
// same origin regardless of domain, port, frp or reverse proxy (the proxy
// must forward the original Host header, which Caddy does by default).
...
function requireSession(request) {
  ...
  const session = openSession(cookies[sessionCookieName], currentSessionSecret());
  return { cookies, session };
}

function requireCsrf(request, cookies) {
  return isAllowedOrigin(request.headers.origin, request.headers.host) && safeEqual(cookies[csrfCookieName], request.headers["x-yomu-csrf"]);
}

async function handleLogin(request, response) {
  if (!isAllowedOrigin(request.headers.origin, request.headers.host)) return json(response, 403, { isSuccess: false, errorMsg: "请求来源无效" });
  let credentials;
  try {
...
async function handleSession(request, response) {
  ...
  const secure = isSecureRequest(request);
  if (!session) return json(response, 401, { isSuccess: false, data: "NEED_LOGIN", errorMsg: "请登录" }, { "set-cookie": clearCookies(request.headers.cookie, secure) });
  try {
    const data = await currentUser(session.token);
    if (!data) return json(response, 401, { isSuccess: false, data: "NEED_LOGIN", errorMsg: "登录已过期" }, { "set-cookie": clearCookies(request.headers.cookie, secure) });
...
async function handleLogout(request, response) {
  ...
  return json(response, 200, { isSuccess: true, data: null }, { "set-cookie": clearCookies(request.headers.cookie, isSecureRequest(request)) });
}
...
function safeSourceCookies(upstream, secure = false) {
  ...
  return raw.flatMap((entry) => {
    ...
    const lower = name.toLowerCase();
    if (!/^[!#$%&'*+.^_`|~0-9a-z-]+$/i.test(name) || lower.startsWith("yomu_") || lower.startsWith("__host-") || lower.startsWith("__secure-")) return [];
    return [`${pair}; Path=/reader3/bookSourceProxy; Max-Age=3600; HttpOnly; SameSite=Lax${secure ? "; Secure" : ""}`];
  });
}

function responseHeaders(upstream, forwardSourceCookies = false, secure = false) {
  const headers = {};
  ...
  if (forwardSourceCookies) {
    const cookies = safeSourceCookies(upstream, secure);
...
async function proxyCore(request, response) {
  ...
  const { cookies, session } = requireSession(request);
  if (!session) return json(response, 401, { isSuccess: false, data: "NEED_LOGIN", errorMsg: "请登录" }, { "set-cookie": clearCookies(request.headers.cookie, isSecureRequest(request)) });
  if (!["GET", "HEAD", "OPTIONS"].includes(request.method || "GET")) {
    ...
    const validMutation = proxyForm ? isAllowedOrigin(request.headers.origin, request.headers.host) : requireCsrf(request, cookies);
  ...
  try {
    ...
    response.writeHead(upstream.status, responseHeaders(upstream, pathname === "/reader3/bookSourceProxy", isSecureRequest(request)));
...
export const server = createServer(async (request, response) => {
  ...
  applySecurityHeaders(response, !isPrivatePath, isSecureRequest(request));
