...
use crate::util::crypto::{gen_encrypted_password, hash_password, random_string, verify_password};
...
impl UserService {
    ...
    pub async fn logout(&self, access_token: &str) -> Result<(), AppError> {
        ...
        query("DELETE FROM user_sessions WHERE username=?1 AND token=?2")
            .bind(&username)
            .bind(&token)
            .execute(&self.pool)
            .await?;
        if user.token == token {
            query("UPDATE users SET token='' WHERE username=?1")
                .bind(&username)
                .execute(&self.pool)
                .await?;
        }
    ...
    pub async fn add_user(&self, username: &str, password: &str) -> Result<Vec<Value>, AppError> {
        ...
        self.validate_new_user(username, password, admin_code)?;
    ...
    pub async fn change_password(
        ...
        let mut user = self
            ...
            .await?
            .ok_or_else(|| AppError::BadRequest("用户不存在".to_string()))?;
        if !verify_password(old_password, &user.password, &user.salt) {
            return Err(AppError::BadRequest("当前密码错误".to_string()));
        }

        let salt = String::new();
        let encrypted = hash_password(new_password)?;
        let now = now_ms();
        user.salt = salt;
        user.password = encrypted;
    ...
    pub async fn delete_users(&self, usernames: &[String]) -> Result<Vec<Value>, AppError> {
        for u in usernames {
            query("DELETE FROM user_sessions WHERE username=?1")
                .bind(u)
                .execute(&self.pool)
                .await?;
            ...
            query("DELETE FROM book_sources WHERE user_ns=?1")
                ...
                .execute(&self.pool)
                .await?;
            query("DELETE FROM json_documents WHERE namespace=?1")
                .bind(u)
                .execute(&self.pool)
                .await?;
    ...
    pub async fn check_auth(&self, access_token: &str) -> Result<Option<User>, AppError> {
        ...
        let row = query("SELECT expire_at FROM user_sessions WHERE username=?1 AND token=?2")
            .bind(&username)
            .bind(&token)
    ...
    async fn update_user_session_fields(
        ...
        query("UPDATE users SET token=?1, last_login_at=?2 WHERE username=?3")
            .bind(token)
            .bind(last_login_at)
            .bind(username)
            .execute(&self.pool)
    ...
    async fn ensure_admin_user(&self) -> Result<(), AppError> {
        let row = query("SELECT username FROM users WHERE is_admin=1 LIMIT 1")
            .fetch_optional(&self.pool)
            .await?;
        if row.is_some() {
            return Ok(());
        ...
        if let Some(row) = row {
            ...
            query("UPDATE users SET is_admin=1 WHERE username=?1")
                .bind(username)
                .execute(&self.pool)
                .await?;
        }
    ...
    async fn delete_expired_sessions(&self, username: &str, now: i64) -> Result<(), AppError> {
        query("DELETE FROM user_sessions WHERE username=?1 AND expire_at<=?2")
            .bind(username)
            .bind(now)
            .execute(&self.pool)
            .await?;
...
mod tests {
    ...
    async fn create_user_service() -> (UserService, PathBuf) {
        let temp_dir =
            std::env::temp_dir().join(format!("yomu-user-service-{}", random_string(8)));
        let cfg = AppConfig {
            secure: true,
    ...
    #[tokio::test]
    async fn migrates_legacy_users_json_to_sqlite_and_keeps_session() {
        let (service, temp_dir) = create_user_service().await;
        let salt = "salt1234".to_string();
        let password = gen_encrypted_password("password1234", &salt);
        ...
        assert!(service.check_auth(access_token).await.unwrap().is_some());
    ...
    async fn login_keeps_legacy_session_and_supports_multi_device() {
        ...
        let first_login = service
            .login("reader1", "password1234", false, None)
            .await
            .unwrap();
        ...
        let second_login = service
            .login("reader1", "password1234", true, None)
            .await
            .unwrap();
    ...
    async fn get_user_info_refreshes_current_session_and_returns_current_token() {
        ...
        let login = service
            .login("reader2", "password1234", false, None)
            .await
            .unwrap();
    ...
    async fn legacy_users_without_admin_promote_earliest_account() {
        ...
        assert!(service
            .is_admin(Some(&first_access_token), None)
            .await
            .unwrap());
    ...
    #[tokio::test]
    async fn migration_reads_legacy_snake_case_user_fields() {
        let (service, temp_dir) = create_user_service().await;
        let salt = "salt1234".to_string();
        let password = gen_encrypted_password("password1234", &salt);
        let users = serde_json::json!({
        ...
        assert!(service
            .is_admin(Some("reader1:legacy-token"), None)
            .await
            .unwrap());
    ...
    #[tokio::test]
    async fn session_refresh_does_not_overwrite_user_changes() {
        let (service, temp_dir) = create_user_service().await;
        ...
        query("UPDATE users SET enable_webdav=1 WHERE username='reader1'")
    ...
    async fn delete_user_removes_sqlite_account_documents() {
        ...
        for (sql, column) in [
            ...
            let row = query(sql).fetch_one(&service.pool).await.unwrap();
    ...
    #[tokio::test]
    async fn concurrent_auth_refreshes_keep_session() {
        let (service, temp_dir) = create_user_service().await;

        let _admin = service
            .login("admin", "password1234", false, None)
            .await
            .unwrap();
        let login = service
            .login("reader1", "password1234", false, None)
