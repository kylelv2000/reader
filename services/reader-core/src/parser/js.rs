...
fn resolve_js_lib_entry(entry: &str) -> anyhow::Result<String> {
    ...
    if value.starts_with("http://") || value.starts_with("https://") {
        return js_http_request(Method::GET, value, None, None);
    }
    Ok(value.to_string())
}
...
fn java_ajax(spec: &str) -> anyhow::Result<String> {
    ...
    let headers = options_json.get("headers").and_then(|v| v.as_object());
    let body = options_json.get("body").and_then(|body| {
    ...
    js_http_request(method, url.trim(), headers, body)
}

fn java_request_simple(method: &str, url: &str, body: Option<String>) -> anyhow::Result<String> {
    let method = Method::from_bytes(method.as_bytes()).unwrap_or(Method::GET);
    js_http_request(method, url.trim(), None, body)
}
...
fn js_http_request(
    ...
    const MAX_JS_HTTP_BODY_BYTES: usize = 5 * 1024 * 1024;
    let mut target = ensure_public_url_blocking(value)?;

    for hop in 0..=5 {
        let mut request = JS_HTTP_CLIENT.request(method.clone(), target.clone());
        ...
        if response.status().is_redirection() {
            ...
            target = ensure_public_url_blocking(next.as_str())?;
        ...
        if response
            .content_length()
            .map(|length| length > MAX_JS_HTTP_BODY_BYTES as u64)
            .unwrap_or(false)
        {
            return Err(anyhow::anyhow!("script HTTP response is too large"));
        }
        let mut bytes = Vec::new();
        response
            .take(MAX_JS_HTTP_BODY_BYTES as u64 + 1)
            .read_to_end(&mut bytes)?;
        if bytes.len() > MAX_JS_HTTP_BODY_BYTES {
            return Err(anyhow::anyhow!("script HTTP response is too large"));
        }
